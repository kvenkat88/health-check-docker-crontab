...
def parse_env_file_form_dict(env='base'):
    ...
    file_path = os.path.join(cur_path, r"./configs/{}.env".format(env))

    with open(file_path, 'r') as fh:
        print("Env file fetched is {}.env".format(env))
        vars_dict = dict(
            tuple(line.split('='))
...
def url_framer_or_formatter(url, host_name_or_ip):
    ...
    if "http" in url or "https" in url:
        if ":" in url.split("//")[1]:
            ...
            print("Port is not available in the url provided(i.e url with hostname mapped) - {}, so using the same url".format(url))
            url_framed = url
    else:
        print("Please provide the valid url with schema like http/https for - {}".format(url))
...
def response_validator(url_dict, host_name_ip, api_endpoint):
    ...
    for key, value in url_dict.items():
        ...
        print("{} Executing request for {}::{} {}".format("#" * 20, key,url_framed, "#" * 20))
        status_code, response_data, error_msg = common_http_validator(method='GET', url=url_framed)
        if status_code == 200:
            print("{} ok status obtained with response message as {}".format(status_code,json.loads(response_data)['status']))
        else:
            print("{} status with response as {} and exception message as {}".format(status_code,response_data,error_msg))

        print("{} Request execution completed for {}::{} {}".format("#" * 20, key,url_framed, "#" * 20))
...
if __name__ == '__main__':
    """
        1. To exceute the script, issue the below mentioned in command prompt or terminal, 
           Example1 :: python request_executor.py 10.1.2.3 -- to run in local
           Example1 :: python request_executor.py os.environ["HOST_INFO"] -- from docker env and os.environ["HOST_INFO"] will have host name info
    """
    host_info_from_env = os.environ["HOST_INFO"]
    print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Crawling Start Time in UTC - {} $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$".format(datetime.strftime(datetime.utcnow(), '%m-%d-%Y-T%H:%M:%S')))
    response_validator(parse_env_file_form_dict(), str(host_info_from_env), str("/healthz"))
    print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ Crawling Stop Time in UTC - {} $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$".format(datetime.strftime(datetime.utcnow(), '%m-%d-%Y-T%H:%M:%S')))
